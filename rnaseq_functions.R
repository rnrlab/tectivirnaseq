...
clusters_expression <- function(expression_matrix, metadata, times,
  ...
  dat <- logratio_long %>%
    ...
    filter(!is.na(Cluster), !is.na(Time))
  ...
  label_dat <- summary_dat %>%
    ...
    summarise(
      ...
      y_at_x = mean_CLR[Time == ((min(times) + max(times)) / 2)][1],
      .groups = "drop"
    ) %>%
    mutate(
      x_pos = (min(times) + max(times)) / 2,
      y_range = y_max - y_min,
      y_mid = (y_max + y_min) / 2,
      y_pos = ifelse(
        y_at_x > y_mid,
  ...
  all_clusters <- ggplot(summary_dat,
                         aes(Time, mean_CLR, group = Cluster, colour = de_score, fill = de_score)) +
    ...
    geom_text(
      ...
      aes(Time, mean_CLR, label = Cluster, colour = de_score),
      hjust = -0.1,
      size = 3,
      show.legend = FALSE
    ) +
    ...
    scale_x_continuous(
      breaks = times,
      limits = c(min(times) - 5, max(times) + 5),
      expand = c(0, 0)
    ) +
    labs(x = "Time (minutes)", y = "CLR counts") +
  ...
  if (!is.null(cluster_colors)) {
    facet_clusters <- ggplot(summary_dat,
                             aes(Time, mean_CLR, group = Cluster,
    ...
    facet_clusters <- ggplot(summary_dat, aes(Time, mean_CLR, group = Cluster)) +
      geom_ribbon(aes(ymin = mean_CLR - ci95, ymax = mean_CLR + ci95),
...
cog_enrichment <- function(genes_set, all_genes) {
  ...
  cog_enrichment_df <- tab_df %>%
    ...
    dplyr::mutate(COG = paste0(COG, " - ", Name)) %>%
    dplyr::select(-Name)
...
create_summary <- function(
  ...
  is_time <- grepl("times", contrast_name, ignore.case = TRUE)
...
combine_summaries <- function(type = c("contrast", "enrichment"), group = c("lysogen", "time"), filter = FALSE){
  type <- match.arg(type)
  ...
  if(group == "lysogen"){
    ...
    stop("Group must be 'lysogen' or 'time'")
  }
...
create_de_genes_table <- function(group = c("lysogen","time"), filter = FALSE){
...
volcano_plot <- function(contrast_df, limits) {
  ...
  contrast_df <- contrast_df %>%
    dplyr::left_join(annotation %>% dplyr::select(Gene_ID, Element),
                     by = c("gene" = "Gene_ID")) %>%
    dplyr::mutate(
  ...
  p <- ggplot(
    ...
    scale_color_manual(values = c("UP" = "#E41A1C",
                                  "DOWN" = "#377EB8",
                                  "N. S." = "lightgrey")) +
    scale_shape_manual(values = c("chromosome" = 19,
                                  "pBtic235" = 15,
                                  "GIL01" = 17,
                                  "GIL16" = 17)) +
    labs(
